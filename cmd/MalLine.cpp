...
void MalLine::ProcessOperation(string &opcode, string &workingCopy)
{
	...
	if (opcode=="ADD" | opcode=="SUB") //Is this an ADD or SUB instruction?
	{
		string args[3];
	...
	else if (opcode=="INC" | opcode=="DEC") //Is this an INC or DEC instruction?
	{
		string args[1];
	...
	else if (opcode=="LOAD" | opcode=="STORE") //Is this a LOAD or STORE instruction?
	{
		string args[2];
	...
	else if (opcode=="LOADI") //Is this a LOADI instruction?
	{
		string args[2];
		_errorCode = ExtractArgs(workingCopy, lastarg, args, 2);
		if (_errorCode == NoError)
	...
	else if (opcode=="BEQ" | opcode=="BGT" | opcode=="BLT") //Is this a BEQ, BLT, or BGT instruction?
	{
		string args[3];
		_errorCode = ExtractArgs(workingCopy, lastarg, args, 3);
		if (_errorCode == NoError)
	...
	else if (opcode=="B") //Is this a B instruction?
	{
		string args[1];
		_errorCode = ExtractArgs(workingCopy, lastarg, args, 1);
		if (_errorCode == NoError)
	...
	else if (opcode=="NOOP" | opcode=="END") //Is this a NOOP or END instruction?
	{
